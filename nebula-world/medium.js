...
function bakeNoise(N = 64) {
  ...
  for (let z = 0; z < N; z++) for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
    ...
    const base = fbm(u, v, w, 1, 2, 4);                                   // R: billowing mass
    const chamber = 1 - Math.min(1, worley(u, v, w, 3, F3) * 1.05);          // G: big cavities (1 at chamber centres)
    const wf = (1 - Math.min(1, worley(u, v, w, 4, F6))) * 0.625 + (1 - Math.min(1, worley(u, v, w, 8, F12))) * 0.25 + (1 - Math.min(1, worley(u, v, w, 16, F24))) * 0.125;  // B: worley-fbm, cauliflower
    const high = fbm(u, v, w, 41, 8, 3);                                    // A: fine grain
...
float cloud(vec3 p){
  ...
  base = clamp(remap(base, (1.0 - n.b) * 0.5, 1.0, 0.0, 1.0), 0.0, 1.0);   // worley-fbm erodes the billows
  float cav = smoothstep(0.15, 0.95, n.g);                             // chambers
  float shape = base - uCarve * cav - uThreshold;
  float dl = length(p - uLightPos);
  shape -= uPocket * exp(-dl*dl / 3000.0);
...
float tangle(vec3 p){
  vec3 c = p - (uLightPos + vec3(15.0, 7.0, 6.0));
...
const P = {
  density: 1.0, carve: 1.2, threshold: 0.22, detail: 0.55, warp: 1.0, absorb: 0.8, scale: 0.005, drift: 0.5,
